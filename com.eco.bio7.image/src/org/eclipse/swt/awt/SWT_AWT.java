...
public class SWT_AWT {
	...
	public static boolean scaleCanvas = false;
	...
	static synchronized void loadLibrary() {
		...
		/*
		 * Note that the jawt library is loaded explicitly because it cannot be found by
		 * the library loader. All exceptions are caught because the library may have
		 * been loaded already.
		 */
		try {
	...
	/**
	 * Returns a <code>java.awt.Frame</code> which is the embedded frame associated
	 * with the specified composite.
	 *
	 * @param parent the parent <code>Composite</code> of the
	...
	/**
	 * Creates a new <code>java.awt.Frame</code>. This frame is the root for the AWT
	 * components that will be embedded within the composite. In order for the
	 * embedding to succeed, the composite must have been created with the
	 * SWT.EMBEDDED style.
	 * <p>
	 * IMPORTANT: As of JDK1.5, the embedded frame does not receive mouse events.
	 * When a lightweight component is added as a child of the embedded frame, the
	 * cursor does not change. In order to work around both these problems, it is
	 ...
	 * </p>
	 *
	 * @param parent the parent <code>Composite</code> of the new
	 *               <code>java.awt.Frame</code>
	 * @return a <code>java.awt.Frame</code> to be the parent of the embedded AWT
	 *         components
	 *
	 * @exception IllegalArgumentException
	 *                                     <ul>
	 ...
	 *                                     parent Composite does not have the
	 *                                     SWT.EMBEDDED style</li>
	 *                                     </ul>
	 *
	 * @since 3.0
	 */
	public static Frame new_Frame(final Composite parent) {
		...
		/*
		 * Generate the appropriate events to activate and deactivate the embedded
		 * frame. This is needed in order to make keyboard focus work properly for
		 * lightweights.
		 */
		Listener listener = e -> {
			switch (e.type) {
			...
			case SWT.Activate:
				EventQueue.invokeLater(() -> {
					...
					try {
						...
						if (method != null)
							method.invoke(frame, Boolean.TRUE);
					} catch (Throwable e1) {
					}
				});
				break;
			case SWT.Deactivate:
				EventQueue.invokeLater(() -> {
					...
					try {
						...
						Method method = clazz.getMethod("synthesizeWindowActivation", boolean.class);
						if (method != null)
							method.invoke(frame, Boolean.FALSE);
					} catch (Throwable e1) {
					}
				});
				break;
		...
		parent.getDisplay().asyncExec(() -> {
			...
			/*Patched for Bio7. Wrong layout in HighDPI displays!*/
			if (scaleCanvas) {
				clientArea = DPIUtil.autoScaleUp(parent.getClientArea()); // To Pixels
			} else {
				clientArea = parent.getClientArea(); // To Pixels
			}
			EventQueue.invokeLater(() -> {
	 ...
	 * that will be embedded within the AWT canvas.
	 *
	 ...
	 * @exception IllegalArgumentException
	 *                                     <ul>
	 *                                     <li>ERROR_NULL_ARGUMENT - if the display
	 *                                     is null</li>
	 *                                     <li>ERROR_NULL_ARGUMENT - if the parent
	 *                                     is null</li>
	 *                                     <li>ERROR_INVALID_ARGUMENT - if the
	 *                                     parent's peer is not created</li>
	 *                                     </ul>
	 *
	 * @since 3.0
	 */
