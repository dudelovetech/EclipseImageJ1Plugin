...
public class ResultsTable implements Cloneable {
	...
	public static final int AREA = 0, MEAN = 1, STD_DEV = 2, MODE = 3, MIN = 4, MAX = 5, X_CENTROID = 6, Y_CENTROID = 7, X_CENTER_OF_MASS = 8, Y_CENTER_OF_MASS = 9, PERIMETER = 10, ROI_X = 11,
			ROI_Y = 12, ROI_WIDTH = 13, ROI_HEIGHT = 14, MAJOR = 15, MINOR = 16, ANGLE = 17, CIRCULARITY = 18, FERET = 19, INTEGRATED_DENSITY = 20, MEDIAN = 21, SKEWNESS = 22, KURTOSIS = 23,
			AREA_FRACTION = 24, RAW_INTEGRATED_DENSITY = 25, CHANNEL = 26, SLICE = 27, FRAME = 28, FERET_X = 29, FERET_Y = 30, FERET_ANGLE = 31, MIN_FERET = 32, ASPECT_RATIO = 33, ROUNDNESS = 34,
			SOLIDITY = 35, MIN_THRESHOLD = 36, MAX_THRESHOLD = 37, LAST_HEADING = 37;
	private static final String[] defaultHeadings = { "Area", "Mean", "StdDev", "Mode", "Min", "Max", "X", "Y", "XM", "YM", "Perim.", "BX", "BY", "Width", "Height", "Major", "Minor", "Angle", "Circ.",
			"Feret", "IntDen", "Median", "Skew", "Kurt", "%Area", "RawIntDen", "Ch", "Slice", "Frame", "FeretX", "FeretY", "FeretAngle", "MinFeret", "AR", "Round", "Solidity", "MinThr", "MaxThr" };
	...
	/**
	 * Adds a value to the end of the given column. If the column does not exist, it
	 * is created. Counter must be >0. There is an example at:<br>
	...
	/**
	 * Adds a string value to the end of the given column. If the column does not
	 * exist, it is created. Counter must be >0.
	 */
	 ...
	 * lable, where 0<=row<counter. After labels are added or modified, call
	 ...
	 * 0&lt;=column&lt;=(lastRow+1 and 0&lt;=row&lt;=counter.
	 */
	 ...
	 * 0&lt;=row&lt;counter. If the specified column does not exist, it is created.
	...
	private String getValueAsString(int column, int row) {
		...
		// IJ.log("getValueAsString1: col="+column+ ", row= "+row+", value= "+value+",
		// size="+stringColumns.size());
		if (Double.isNaN(value) && stringColumns != null) {
			String string = "NaN";
			ArrayList stringColumn = (ArrayList) stringColumns.get(new Integer(column));
			if (stringColumn == null)
	...
	public void update(int measurements, ImagePlus imp, Roi roi) {
		...
		for (int i = 0; i <= last; i++) {
			// IJ.log(i+" "+rt2.getColumn(i)+" "+columns[i]+" "+rt2.getColumnHeading(i)+"
			// "+getColumnHeading(i));
