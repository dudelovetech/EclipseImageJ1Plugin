 ...
 *
 *
 * Notes on fitting polynomial functions: (i) The range of x values should not
 * be too far from 0, especially for higher-order polynomials. For polynomials
 * of 4th order, the average x value should fulfill |xMean| < 2*(xMax-xMin). For
 * polynomials of 5th order or higher, the x range should encompass x=0; and for
 ...
 * matrix inversion, the simplex algorithm used here can cope with parameters
 * having very different orders of magnitude, as long as the coefficients of the
 * polynomial are within a reasonable range (say, 1e-80 to 1e80). Thus, it is
 * usually not needed to scale the x values, even for high-order polynomials.
 *
 * Version history:
 *
 * 2008-01-21: Modified to do Gaussian fitting by Stefan Woerz (s.woerz at
 * dkfz.de). 2012-01-30: Modified for external Minimizer class and UserFunction
 * fit by Michael Schmid. - Also fixed incorrect equation string for 'Gamma
 * Variate' & 'Rodbard (NIH Image)', - Added 'Inverse Rodbard', 'Exponential
 * (linear regression)', 'Power (linear regression)' functions and polynomials
 * of order 5-8. Added 'nicely' sorted list of types. - Added absolute error for
 * minimizer to avoid endless minimization if exact fit is possible. - Added
 * 'initialParamVariations' (order of magnitude if parameter variations) - this
 * is important for safer and better convergence. - Linear regression for up to
 * 2 linear parameters, reduces the number of parameters handled by the simplex
 * Minimizer and improves convergence. These parameters can be an offset and
 * either a linear slope or a factor that the full function is multiplied with.
 * 2012-10-07: added GAUSSIAN_NOOFFSET fit type 2012-11-20: Bugfix: exception on
 * Gaussian&Rodbard with initial params, bad initial params for Gaussian
 * 2013-09-24: Added "Exponential Recovery (no offset)" and "Chapman-Richards"
 * (3-parameter; used e.g. to describe forest growth) fit types. 2013-10-11:
 * bugfixes, added setStatusAndEsc to show iterations and enable abort by ESC
 * 2015-03-26: bugfix, did not use linear regression for RODBARD 2016-11-28:
 * added static getNumParams methods 2018-03-23: fixes NullPointerException for
 * custom fit without initialParamVariations 2018-07-19: added error function
 * erf (=integral over Gaussian)
 */

public class CurveFitter implements UserFunction {
	/** Constants for the built-in fit functions */
	public static final int STRAIGHT_LINE = 0, POLY2 = 1, POLY3 = 2, POLY4 = 3, EXPONENTIAL = 4, POWER = 5, LOG = 6, RODBARD = 7, GAMMA_VARIATE = 8, LOG2 = 9, RODBARD2 = 10, EXP_WITH_OFFSET = 11, GAUSSIAN = 12, EXP_RECOVERY = 13, INV_RODBARD = 14, EXP_REGRESSION = 15, POWER_REGRESSION = 16,
			POLY5 = 17, POLY6 = 18, POLY7 = 19, POLY8 = 20, GAUSSIAN_NOOFFSET = 21, EXP_RECOVERY_NOOFFSET = 22, CHAPMAN = 23, ERF = 24;

	/** Nicer sequence of the built-in function types */
	public static final int[] sortedTypes = { STRAIGHT_LINE, POLY2, POLY3, POLY4, POLY5, POLY6, POLY7, POLY8, POWER, POWER_REGRESSION, EXPONENTIAL, EXP_REGRESSION, EXP_WITH_OFFSET, EXP_RECOVERY, EXP_RECOVERY_NOOFFSET, LOG, LOG2, GAUSSIAN, GAUSSIAN_NOOFFSET, ERF, RODBARD, RODBARD2, INV_RODBARD,
			GAMMA_VARIATE, CHAPMAN };

	/** Names of the built-in fit functions */
	public static final String[] fitList = { "Straight Line", "2nd Degree Polynomial", "3rd Degree Polynomial", "4th Degree Polynomial", "Exponential", "Power", "Log", "Rodbard", "Gamma Variate", "y = a+b*ln(x-c)", "Rodbard (NIH Image)", "Exponential with Offset", "Gaussian",
			"Exponential Recovery", "Inverse Rodbard", "Exponential (linear regression)", "Power (linear regression)", "5th Degree Polynomial", "6th Degree Polynomial", "7th Degree Polynomial", "8th Degree Polynomial", "Gaussian (no offset)", "Exponential Recovery (no offset)",
			"Chapman-Richards", "Error Function" }; // fList, doFit(), getNumParams() and makeInitialParamsAndVariations() must also
								// be updated

	/** Equations of the built-in fit functions */
	public static final String[] fList = { "y = a+bx", "y = a+bx+cx^2", // STRAIGHT_LINE,POLY2
			"y = a+bx+cx^2+dx^3", "y = a+bx+cx^2+dx^3+ex^4", "y = a*exp(bx)", "y = a*x^b", "y = a*ln(bx)", // EXPONENTIAL,POWER,LOG
			...
			"y = a+bx+cx^2+dx^3+ex^4+fx^5", "y = a+bx+cx^2+dx^3+ex^4+fx^5+gx^6", "y = a+bx+cx^2+dx^3+ex^4+fx^5+gx^6+hx^7", "y = a+bx+cx^2+dx^3+ex^4+fx^5+gx^6+hx^7+ix^8", "y = a*exp(-(x-b)*(x-b)/(2*c*c)))", // GAUSSIAN_NOOFFSET
	...
	/**
	 * @deprecated now in the Minimizer class (since ImageJ 1.46f). (probably of not
	 *             much value for anyone anyhow?)
	 */
	...
	/**
	 * Perform curve fitting with one of the built-in functions doFit(fitType) does
	 * the fit quietly Use getStatus() and/or getStatusString() to see whether
	 * fitting was (probably) successful and getParams() to access the result.
	 */
	...
	/**
	 * Perform curve fitting with one of the built-in functions doFit(fitType, true)
	 * pops up a dialog allowing the user to set the initial fit parameters and
	 * various numbers controlling the Minimizer Use getStatus() and/or
	 * getStatusString() to see whether fitting was (probably) successful and
	...
	public void doFit(int fitType, boolean showSettings) {
		...
		// IJ.log("special params: off="+offsetParam+(hasSlopeParam ? " slo=" : "
		...
		if (this.fitType == STRAIGHT_LINE) { // no minimizer needed
			...
			// ["+ii+"]:"+IJ.d2s(initialParamVariations[ii],5,9);IJ.log(s);}
			// {String s="minInitVariations:";for(int ii=0;ii<numParams;ii++)s+="
			// ["+ii+"]:"+IJ.d2s(minimizerInitialParamVariations[ii],5,9);IJ.log(s);}
			// {String s="minInitPars:";for(int ii=0;ii<numParams;ii++)s+="
			// ["+ii+"]:"+IJ.d2s(minimizerInitialParams[ii],5,9);IJ.log(s);}
	 ...
	 * the number of parameters, or 0 in case of a macro syntax error.
	 *
	 ...
	 * or more of the initialParams are zero). Use getStatus() and/or
	 * getStatusString() to see whether fitting was (probably) successful and
	 * getParams() to access the result.
	 */
	 ...
	 *
	 * @param userFunction           A plugin where the fit function is defined by
	 *                               the userFunction(params, x) method. This
	 *                               function must allow simultaneous calls in
	 *                               multiple threads.
	 * @param numParams              Number of parameters of the fit function.
	 * @param formula                A String describing the fit formula, may be
	 *                               null.
	 * @param initialParams          Starting point for the parameters; may be null
	 *                               (than values of 0 are used). The fit function
	 *                               with these parameters must not return NaN for
	 *                               any of the data points given in the constructor
	 *                               (xData).
	 * @param initialParamVariations Each parameter is initially varied by up to +/-
	 *                               this value. If not given (null), initial
	 *                               variations are taken as 10% of initial
	 *                               parameter value or 0.01 for parameters that are
	 *                               zero. When this array is given, all elements
	 *                               must be positive (nonzero). See
	 *                               Minimizer.minimize for details.
	 * @param showSettings           Displays a popup dialog for modifying the
	 *                               initial parameters and a few numbers
	 ...
	 * directly. Note that no Minimizer is used if fitType is any of STRAIGHT_LINE,
	 * EXP_REGRESSION, and POWER_REGRESSION.
	 */
	 ...
	 * specify parameters that can be calculated directly by linear regression. For
	 * values not used, set the index to -1
	 *
	 * @param offsetParam   Index of a parameter that is a pure offset: E.g. '0' if
	 *                      f(p0, p1, p2...) = p0 + function(p1, p2, ...).
	 * @param multiplyParam Index of a parameter that is purely multiplicative. E.g.
	 *                      multiplyParams=1 if f(p0, p1, p2, p3...) can be
	 *                      expressed as p1*func(p0, p2, p3, ...) or p0 +p1*func(p0,
	 *                      p2, p3, ...) with '0' being the offsetparam.
	 * @param slopeParam    Index of a parameter that is multiplied with x and then
	 *                      summed to the function. E.g. '1' for f(p0, p1, p2,
	 *                      p3...) = p1*x + func(p0, p2, p3, ...) Only one,
	 *                      multiplyParam and slopeParam can be used (ie.e, the
	 *                      other should be set to -1)
	 */
	...
	/**
	 * Get number of parameters for current fit formula Do not use before 'doFit',
	 * because the fit function would be undefined.
	 */
	 ...
	 * 'custom' fit, where the number of parameters is given by the equation: see
	 * getNumParams(String)
	 */
	 ...
	 * String, like "y = a + b*x + c*x*x" . Restricted to 6 parameters "a" ... "f"
	...
	/**
	 * Returns the formula value for parameters 'p' at 'x'. Do not use before
	 * 'doFit', because the fit function would be undefined.
	 */
	 ...
	 * the rest. May return an array with only NaN values if the minimizer could not
	 * start properly, i.e., if getStatus() returns Minimizer.INITILIZATION_FAILURE.
	...
	/**
	 * Returns residuals array, i.e., differences between data and curve. The
	 * residuals are with respect to the real data, also for fit types where the
	 * data are modified before fitting (power&exp fit by linear regression,
	 * 'Rodbard NIH Image' ). This is in contrast to sum of squared residuals, which
	 * is for the fit that was actually done.
	 */
	 ...
	 * properly i.e., if getStatus() returns Minimizer.INITILIZATION_FAILURE).
	 */
	...
	/**
	 * Returns the standard deviation of the residuals. Here, the standard deviation
	 * is defined here as the root-mean-square of the residuals times sqrt(n/(n-1));
	 ...
	 * calculated for the fit actually done, not for the residuals of the original
	 * data.
	 */
	...
	/**
	 * Get a measure of "goodness of fit" where 1.0 is best. Approaches R^2 if the
	 * number of points is much larger than the number of fit parameters. For power,
	 * exp by linear regression and 'Rodbard NIH Image', this is calculated for the
	 ...
	 * over Minimizer.STATUS_STRING[getMinimizer().getStatus()] because
	 * getStatusString() better explains the problem in some cases of initialization
	 * failure (data not compatible with the fit function chosen)
	 */
	 ...
	 * lines, including the fit parameters).
	 */
	 ...
	 * true value (relative w.r.t. full sum of rediduals). Possible range: 0.1 ...
	 * 10^-16
	 */
	 ...
	 *                       display if null. E.g. "Curve Fit: Iteration "
	 * @param checkEscape    When true, the Minimizer stops if escape is pressed and
	 *                       the status becomes ABORTED. Note that checking for ESC
	 *                       does not work in the Event Queue thread.
	 */
	 ...
	 * a successful completion. In case of Minimizer.INITIALIZATION_FAILURE, fitting
	 * could not be performed because the data and/or initial parameters are not
	 * compatible with the function value. In that case, getStatusString may explain
	 * the problem. For further status codes indicating problems during fitting, see
	 * the status codes of the Minimzer class.
	 */
	 ...
	 * residuals for given parameters. To improve the efficiency, simple linear
	 * dependencies are solved directly by linear regression; in that case the
	 * corresponding parameters are modified. This effectively reduces the number of
	 * free parameters by one or two and thereby significantly improves the
	 * performance of minimization.
	 */
	public final double userFunction(double[] params, double dummy) {
		...
		if (numRegressionParams == 0) { // simply calculate sum of residuals
			...
			// IJ.log(IJ.d2s(params[0],3,5)+","+IJ.d2s(params[1],3,5)+":
			...
			// ii=0;ii<=numParams;ii++)s+=" ["+ii+"]:"+IJ.d2s(params[ii],5,9);IJ.log(s);}
	 ...
	 * handled by the Minimizer, convert Minimizer parameters to the complete set of
	 * parameters. When not for calculating regression, we use the sum of squared
	 * residuals, offset and multiplication factor stored in the extra array
	 * elements: The Minimizer stores the sum of squared residuals directly behind
	 * its last parameter. The next element is the value of the offsetParam (if
	 * any). The final element is the value of the factorParam (if any).
	 */
	private void minimizerParamsToFullParams(double[] params, boolean forRegression) {
		boolean shouldTransformToSmallerParams = false;
		double offset = 0;
		double factor = hasSlopeParam ? 0 : 1; // for regression, calculate function value without x*slope, but multiplied with
							// 1
	...
	/**
	 * Determine sum of squared residuals with linear regression. The sum of squared
	 * residuals is written to the array element with index 'numParams', the offset
	 * and factor params (if any) are written to their proper positions in the
	 * params array
	 */
	private void doRegression(double[] params) {
		...
		for (int i = 0; i < numPoints; i++) {
			...
			// if(getIterations()==0)IJ.log(xData[i]+"\t"+yData[i]+"\t"+fValue);
			// //x,y,function
		...
		if (offsetParam < 0) { // only 'factor' parameter, no offset:
			...
			// if(){IJ.log("sumX="+sumX+" sumX2="+sumX2+" sumXY="+sumXY+" factor="+factor+"
			// offset=="+offset);}
		}
		params[numParams] = sumResidualsSqr;
		if (factorParam >= 0)
	 ...
	 * residuals squared. The last array elements, not used by the minimizer, are
	 * the offset and factor parameters (if any)
	 */
	 ...
	 * minimizer: Make modified initialParams and initialParamVariations for the
	 * Minimizer
	 */
	 ...
	 * built-in function types. For fits with modifications for regression,
	 * 'fitType' is still the type of the original (unmodified) fit. Also checks for
	 * x values being non-negative for fit types that require this, and returns
	 * false if the data cannot be fitted because of negative x. In such a case,
	 * 'errorString' contains a message about the problem.
	 */
	private boolean makeInitialParamsAndVariations(int fitType) {
		...
		if (!hasInitialParams) {
			...
			case EXP_RECOVERY: // a*(1-exp(-bx)) + c
				...
				initialParams[0] = (yMax - yMin + 1e-100) / Math.exp(initialParams[1] * xMean) * Math.signum(slope) * fitType == EXP_RECOVERY ? 1 : -1; // don't care, we will do this via regression
	 ...
	 * use linear regression, reducing the number of parameters used by the
	 * Minimizer by up to 2, and improving the speed and success rate of the
	...
	private boolean isModifiedFitType(int fitType) {
		return fitType == POWER_REGRESSION || fitType == EXP_REGRESSION || fitType == RODBARD || fitType == RODBARD2 || fitType == GAUSSIAN;
	}

	/**
	 * For fits don't use the original data, prepare modified data and fit type.
	 * Returns false if the data are incompatible with the fit type In that case,
	 * 'errorString' is set to a message explaining the problem
	 */
