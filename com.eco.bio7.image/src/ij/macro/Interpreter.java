...
public class Interpreter implements MacroConstants {
	...
	/**
	 * Runs the specified macro, passing it a string argument and returning a string
	 * value.
	 */
	...
	/*
	 * public void eval(String macro) { Tokenizer tok = new Tokenizer(); this.pgm =
	 * tok.tokenize(macro); evaluating = true; pushGlobals(); if (func==null) func =
	 * new Functions(this, pgm); run(0); }
	 */
	...
	/**
	 * Searches the entire stack for the variable associated with the current token.
	...
	public void showArrayInspector(int row) {
		...
		if (stack.length > stkPos && stkPos >= 0) {
			...
			if (var.getType() == Variable.ARRAY) {
				...
				String oldText = txtPanel.getText();// n__ possible NullPointer at ij.text.TextPanel.getText(TextPanel.java:875)
													// vData == null
