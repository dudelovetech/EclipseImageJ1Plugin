...
public class Compiler implements PlugIn, FilenameFilter {
	...
	boolean isJavac() {
		...
		try {
			...
			// bundle =com.eco.bio7.image.Activator.getDefault().getBundle().getHeaders().get("Bundle-ClassPath").toString();
	...
	boolean compile(String path) {
		...
		String fileDirectory = binPath+File.pathSeparator + f.getParent()+ File.pathSeparator+File.pathSeparator + f.getParent();
