...
public class CommandFinder implements PlugIn, ActionListener, WindowListener, KeyListener, ItemListener, MouseListener {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (source == runButton) {
			...
			String text = "<html>Shortcuts:<br>" + "&emsp;&uarr; &darr;&ensp; Select items<br>" + "&emsp;&crarr;&emsp; Open item<br>" + "&ensp;A-Z&ensp; Alphabetic scroll<br>"
	...
	public void keyPressed(KeyEvent ke) {
		...
		if (key == KeyEvent.VK_ESCAPE || (key == KeyEvent.VK_W && meta)) {
			...
			/*
			 * If someone presses backspace they probably want to remove the last letter
			 * from the search string, so switch the focus back to the prompt:
			 */
			prompt.requestFocus();
	 ...
	 *
	 * @param initialSearch
	 *            The search string that populates Command Finder's search field. It
	 *            is ignored if contains an invalid query (ie, if it is either
