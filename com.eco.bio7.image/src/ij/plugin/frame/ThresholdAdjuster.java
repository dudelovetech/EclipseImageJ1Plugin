...
public class ThresholdAdjuster extends PlugInDialog implements PlugIn, Measurements, Runnable, ActionListener, AdjustmentListener, ItemListener, FocusListener, KeyListener, MouseWheelListener, ImageListener {
	...
	ImageProcessor setup(ImagePlus imp, boolean enableAutoThreshold) {
		...
		if (minMaxChange || id != previousImageID || type != previousImageType || imageWasUpdated || roiHashCode != previousRoiHashCode) {
			minThreshold = ip.getMinThreshold();
			maxThreshold = ip.getMaxThreshold();
			boolean isThreshold = minThreshold != ImageProcessor.NO_THRESHOLD && ip.getCurrentColorModel() != ip.getColorModel(); // does not work???
	...
	void updatePercentiles(ImagePlus imp, ImageProcessor ip) {
		...
		if (maxThresholdInt > 255)
			maxThresholdInt = 255;
		if (stats != null && stats.histogram != null && stats.histogram.length == 256 && ip.getMinThreshold() != ImageProcessor.NO_THRESHOLD) {
			...
			int digits = imp.getCalibration() != null || (ip instanceof FloatProcessor) ? Math.max(Analyzer.getPrecision(), 2) : 0;
			if (mode == OVER_UNDER)
	...
	void doSet(ImagePlus imp, ImageProcessor ip) {
		/*
		 * double level1 = ip.getMinThreshold(); double level2 = ip.getMaxThreshold(); if (level1==ImageProcessor.NO_THRESHOLD) { level1 = scaleUp(ip, defaultMinThreshold); level2 = scaleUp(ip,
		 * defaultMaxThreshold); }
		 */
		Calibration cal = imp.getCalibration();
		int digits = (ip instanceof FloatProcessor) || cal.calibrated() ? 2 : 0;
		/*
		 * level1 = cal.getCValue(level1); level2 = cal.getCValue(level2); GenericDialog gd = new GenericDialog("Set Threshold Levels"); gd.addNumericField("Lower Threshold Level: ", level1, digits);
		 * gd.addNumericField("Upper Threshold Level: ", level2, digits); gd.showDialog(); if (gd.wasCanceled()) return; level1 = gd.getNextNumber(); level2 = gd.getNextNumber();
		 */
	...
	/**
	 * Notifies the ThresholdAdjuster that the image has changed. If the image has no threshold, it does not autothreshold the image.
	 */
