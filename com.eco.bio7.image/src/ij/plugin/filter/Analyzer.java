...
public class Analyzer implements PlugInFilter, Measurements {
	...
	private static final int[] list = { AREA, MEAN, STD_DEV, MODE, MIN_MAX, CENTROID, CENTER_OF_MASS, PERIMETER, RECT, ELLIPSE, SHAPE_DESCRIPTORS, FERET, INTEGRATED_DENSITY, MEDIAN, SKEWNESS, KURTOSIS, AREA_FRACTION, STACK_POSITION, LIMIT, LABELS, INVERT_Y, SCIENTIFIC_NOTATION, ADD_TO_OVERLAY,
			NaN_EMPTY_CELLS };
	...
	/*
	 * void showHeadings() { String[] headings = rt.getHeadings(); int columns = headings.length; if (columns==0) return;
	 * IJ.log("Headings: "+headings.length+" "+rt.getColumnHeading(ResultsTable.LAST_HEADING)); for (int i=0; i<columns; i++) { if (headings[i]!=null)
	...
	/**
	 * Returns <code>true</code> if an image is selected in the "Redirect To:" popup menu of the Analyze/Set Measurements dialog box.
	 */
	...
	public void saveResults(ImageStatistics stats, Roi roi) {
		...
		if ((measurements & FERET) != 0) {
			boolean extras = true;
			double FeretDiameter = Double.NaN, feretAngle = Double.NaN, minFeret = Double.NaN, feretX = Double.NaN, feretY = Double.NaN;
			Roi roi2 = roi;
