...
public class ImagePlus implements ImageObserver, Measurements, Cloneable {
	 ...
	 * true if the image was successfully locked. Beeps, displays a message in
	 * the status bar, and returns false if the image is already locked.
	 */
	...
	public synchronized void updateAndDraw() {
		if (stack != null && !stack.isVirtual() && currentSlice >= 1 && currentSlice <= stack.getSize()) {
			...
			if (ip != null && pixels != null && pixels != ip.getPixels()) { // was
																			// stack
																			// updated?
				try {
					ip.setPixels(pixels);
	...
	/**
	 * Returns the display mode (IJ.COMPOSITE, IJ.COLOR or IJ.GRAYSCALE) if this
	 * is a composite color image, or 0 if it not.
	 */
	 ...
	 * 'channels' is a list of ones and zeros that specify the channels to
	...
	/**
	 * Returns a reference to the current ImageProcessor. The CompositeImage
	 * class overrides this method to return the processor associated with the
	 * current channel.
	 */
	 ...
	 * also repaints the image window to force the information displayed above
	...
	/**
	 * ImageCanvas.paint() calls this method when the ImageProcessor has
	 * generated new image.
	 */
	...
	/**
	 * Replaces this image with the specified ImagePlus. May not work as
	 * expected if 'imp' is a CompositeImage and this image is not.
	 */
	...
	/**
	 * Replaces the ImageProcessor with the one specified and updates the
	 * display. With stacks, the ImageProcessor must be the same type as other
	 * images in the stack and it must be the same width and height. Set 'title'
	 * to null to leave the title unchanged.
	 */
	...
	public void setStack(ImageStack newStack, int channels, int slices, int frames) {
		...
		if (IJ.debugMode)
			IJ.log("setStack: " + newStack.getSize() + " " + channels + " (" + channelsBefore + ") " + slices + " " + frames + " " + isComposite());
	 ...
	 * ImageProcessor, it creates one. Returns null if this ImagePlus contains
	 * no ImageProcessor and no AWT Image. Sets the line width to the current
	 * line width and sets the calibration table if the image is density
	 * calibrated.
	 */
	...
	/**
	 * Returns an ImageStatistics object generated using the specified
	 * measurement options and histogram bin count. Note: except for float
	 * images, the number of bins is currently fixed at 256.
	 */
	...
	/**
	 * Returns an ImageStatistics object generated using the specified
	 * measurement options, histogram bin count and histogram range. Note: for
	 * 8-bit and RGB images, the number of bins is fixed at 256 and the
	 * histogram range is always 0-255.
	 */
	...
	/**
	 * Returns a shortened version of image name that does not include spaces or
	...
	/**
	 * If this is a stack, returns the actual number of images in the stack,
	 * else returns 1.
	 */
	...
	/**
	 * Sets the 3rd, 4th and 5th dimensions, where <code>nChannels</code>*
	 * <code>nSlices</code>*<code>nFrames</code> must be equal to the stack
	 * size.
	 */
	public void setDimensions(int nChannels, int nSlices, int nFrames) {
		...
		if (nChannels * nSlices * nFrames != getImageStackSize() && ip != null) {
			// throw new
			// IllegalArgumentException("channels*slices*frames!=stackSize");
			nChannels = 1;
	 ...
	 * 'key', or null if the key is not found. Works with DICOM tags and
	 ...
	 * 'key', or NaN if the key is not found or the value associated with the
	 * key is not numeric. Works with DICOM tags and Bio-Formats metadata.
	 * 
	...
	/**
	 * Adds a key-value pair to this image's properties. The key is removed from
	 * the properties table if value is null.
	 */
	 ...
	 * are retuned in the first element. RGB values are returned in the first 3
	...
	/**
	 * Returns the image stack. The stack may have only one slice. After adding
	 * or removing slices, call <code>setStack()</code> to update the image and
	...
	/**
	 * Returns the current stack index (one-based) or 1 if this is a single
	 * image.
	 */
	...
	/**
	 * Returns that stack index (one-based) corresponding to the specified
	 * position.
	 */
	...
	/**
	 * Set the current hyperstack position based on the stack index 'n'
	 * (one-based).
	 */
	...
	/**
	 * Converts the stack index 'n' (one-based) into a hyperstack position
	 * (channel, slice, frame).
	 */
	...
	/**
	 * Displays the specified stack image, where 1<=n<=stackSize. Does nothing
	 * if this image is not a stack.
	 * 
	 * @see #setPosition
	 * @see #setC
	 * @see #setZ
	...
	/**
	 * Assigns 'newRoi' to this image and displays it if 'updateDisplay' is
	 * true.
	 */
	 ...
	 * starting screen coordinates. The selection type is determined by which
	 * tool in the tool bar is active. The user interactively sets the selection
	 * size and shape.
	 */
	...
	/**
	 * Deletes the current region of interest. Makes a copy of the ROI so it can
	 * be recovered by Edit/Selection/Restore Selection.
	 */
	...
	public void restoreRoi() {
		if (Roi.previousRoi != null) {
			...
			if (r.width <= width || r.height <= height || (r.x < width && r.y < height) || isSmaller(pRoi)) { // will
																												// it
																												// (mostly)
																												// fit
																												// in
				...
				roi = (Roi) pRoi.clone();
				roi.setImage(this);
				if (r.x >= width || r.y >= height || (r.x + r.width) < 0 || (r.y + r.height) < 0) // does
																									// it
																									// need
																									// to
																									// be
				...
				else if (r.width == width && r.height == height) // is it the
																	// same size
																	// as the
																	// image
					roi.setLocation(0, 0);
	...
	/**
	 * Returns the FileInfo object that was used to open this image. Returns
	 * null for images created using the File/New command.
	 * 
	 * @see ij.io.FileInfo
	 * @see #getFileInfo
	 */
	 ...
	 * Does nothing if the image is locked.
	 */
	...
	/**
	 * Returns a new ImagePlus with this image's attributes (e.g. spatial
	 * scale), but no image.
	 */
	...
	public ImagePlus createHyperStack(String title, int channels, int slices, int frames, int bitDepth) {
		...
		ImageStack stack2 = new ImageStack(width, height, size); // create empty
																	// stack
		...
		stack2.setPixels(ip2.getPixels(), 1); // can't create ImagePlus will
												// null 1st image
	...
	/**
	 * Copies attributes (name, ID, calibration, path) of the specified image to
	 * this image.
	 */
	 ...
	 * in the status bar. Called by the Next Slice and Previous Slice commands
	 * to update the z-coordinate and pixel value.
	 */
	...
	/**
	 * Copies the contents of the current selection, or the entire image if
	 * there is no selection, to the internal clipboard.
	 */
	public void copy() {
		copy(false);
	}
	 ...
	 * Copies the entire image if there is no selection. Also clears the
	 * selection if <code>cut</code> is true.
	 */
	 ...
	 * there is a selection the same size as the image on the clipboard, the
	 * image is inserted into that selection, otherwise the selection is
	 * inserted into the center of the image.
	 */
	public void paste() {
		if (clipboard == null)
			return;
	...
	/**
	 * Returns the internal clipboard or null if the internal clipboard is
	 * empty.
	 */
	...
	/**
	 * Returns the display mode (IJ.COMPOSITE, IJ.COLOR or IJ.GRAYSCALE) if this
	 * is a CompositeImage, otherwise returns -1.
	 */
	...
	/**
	 * Sets the display range of the current channel. With non-composite images
	 * it is identical to ip.setMinAndMax(min, max).
	 */
	...
	/**
	 * Sets the display range of specified channels in an RGB image, where
	 * 4=red, 2=green, 1=blue, 6=red+green, etc. With non-RGB images, this
	 * method is identical to setDisplayRange(min, max). This method is used by
	 * the Image/Adjust/Color Balance tool .
	 */
	...
	/**
	 * Returns the default 16-bit display range, 0 (auto-scaling), 8, 10, 12,
	 * 14, 15 or 16.
	 */
	 ...
	 *             <br>
	 *             or Java version is less than 1.6. Copied from OverlayCommands
	 *             and modified by Marcel Boeglin on 2014.01.08 to work with
	 *             HyperStacks.
	 */
	...
	/**
	 * Flattens Overlay 'overlay' on slice 'slice' corresponding to coordinates
	 * 'z' and 't' in RGB-HyperStack 'stack'
	 */
	...
	/**
	 * Installs a list of ROIs that will be drawn on this image as a
	...
	/**
	 * Returns the current overly, or null if this image does not have an
	 * overlay.
	 */
