 ...
 * terms of java.awt.Shape. A ShapeRoi is constructed from a
 * <code>ij.gui.Roi</code> object, or as a result of logical operators (i.e.,
 * union, intersection, exclusive or, and subtraction) provided by this class.
 * These operators use the package <code>java.awt.geom</code> as a backend. <br>
...
public class ShapeRoi extends Roi {
	 ...
	 * in Roi objects of type {@link ij.gui.Roi#POLYLINE} and
	 * {@link ij.gui.Roi#POLYGON} for open and closed shapes, respectively.
	 * Conversion of shapes open and closed with more than MAXPOLY line segments
	 * will result, respectively, in {@link ij.gui.Roi#FREELINE} and
	 * {@link ij.gui.Roi#FREEROI} (or {@link ij.gui.Roi#TRACED_ROI} if
	 * {@link #forceTrace} flag is <strong><code>true</code></strong>.
	 */
	 ...
	 * the length of the curve segments of this ROI's shape.
	 */
	 ...
	 * (sub)paths having more than <code>MAXPOLY</code> and composed exclusively of
	 * line segments. If <strong><code>true</code></strong> then (sub)path will be
	 * parsed into a {@link ij.gui.Roi#TRACED_ROI}; else, into a
	 * {@link ij.gui.Roi#FREEROI}.
	 */
	private boolean forceTrace = false;

	/**
	 * Flag which specifies if Roi objects constructed from open (sub)paths composed
	 * of only two line segments will be of type {@link ij.gui.Roi#ANGLE}. If
	 * <strong><code>true</code></strong> then (sub)path will be parsed into a
	 * {@link ij.gui.Roi#ANGLE}; else, into a {@link ij.gui.Roi#POLYLINE}.
	 */
	 ...
	 *                   this ROI will be flattened (i.e., curve segments will be
	 *                   aproximated by line segments).
	 * @param maxPoly    Roi objects constructed from shapes composed of linear
	 *                   segments fewer than this value will be of type
	 *                   {@link ij.gui.Roi#POLYLINE} or {@link ij.gui.Roi#POLYGON};
	 *                   conversion of shapes with linear segments more than this
	 *                   value will result in Roi objects of type
	...
	/**
	 * Unary exclusive or operator. The caller is set to the non-overlapping regions
	 * between the operands.
	 * 
	 * @return the union of the non-overlapping regions of
	 *         <strong><code>this</code></strong> and <code>sr</code>
	 */
	...
	/**
	 * Unary subtraction operator. The caller is set to the result of the operation
	 * between the operands.
	 * 
	 ...
	 * <th scope=col>Winding<br>
	 * rule</th>
	 * <th scope=col>Flag<br>
	 * forceAngle</th>
	 * <th scope=col>Flag<br>
	 * forceTrace</th>
	 * <th scope=col>Flag<br>
	 ...
	 * <td>ij.gui.PolygonRoi</td>
	 * <td>Roi.FREEROI</td>
	 * <td>closed java.awt.geom.GeneralPath</td>
	 * <td>GeneralPath.WIND_EVEN_ODD</td>
	 * <td>false</td>
	 * <td>false</td>
	 * <td>false</td>
	 * </tr>
	 ...
	 * <td>GeneralPath.WIND_NON_ZERO</td>
	 * <td>false</td>
	 * <td>false</td>
	 * <td>false</td>
	 * </tr>
	 ...
	 * @param linesOnly  Indicates wether the GeneralPath object is composed only of
	 *                   SEG_LINETO segments
	 * @param curvesOnly Indicates wether the GeneralPath object is composed only of
	 *                   SEG_CUBICTO and SEG_QUADTO segments
	...
	/**
	 * Caculates "Feret" (maximum caliper width) and "MinFeret" (minimum caliper
	 * width).
	 */
	...
	/**
	 * Returns the perimeter if this ShapeRoi can be decomposed into simple ROIs,
	...
	/**
	 * Length of the chord of the arc of the cubic B&eacute;zier curve argument, in
	 * double precision.
	 */
	...
	/**
	 * Length of the chord of the arc of the quadratic B&eacute;zier curve argument,
	 * in double precision.
	 */
	 ...
	 * <strong>8:</strong><em>13-31</em> (1997)&quot; implemented using
	 * <code>java.awt.geom.CubicCurve2D.Double</code>. Please visit
	 * {@link <a href="http://www.graphicsgems.org/gems.html#gemsiv">Graphics Gems
	 * IV</a>} for examples of other possible implementations in C and C++.
	 */
	...
	// pairs
	/**
	 * Scales a coordinate array with the size calibration of a 2D image. The array
	...
	void scaleCoords(double[] c, double pw, double ph) {
		int k = c.length / 2;
		if (2 * k != c.length)
			return; // bail out if array has odd length
	 ...
	 * array contains a sequence of variable length segments that use from from one
	 * to seven elements. The first element of a segment is the type as defined in
	 * the PathIterator interface. SEG_MOVETO and SEG_LINETO segments also include
	 * two coordinates, SEG_QUADTO segments include four coordinates and SEG_CUBICTO
	 * segments include six coordinates.
	 */
	 ...
	 * and returns several convenience parameters in the arguments. Iterates through
	 * the PathIterator argument and as a byproduct sets the values of the other
	 * arguments passed to the method. To retrieve, for example, the length (or
	 * perimeter) of a shape (or path), call this method as<br>
	 * <code> parsePath(pIter, par, null, null) <code>,
	 * <br> where <code>par</code> is a double array of length one, then get the
	 * length as <code>par[0]</code>
	 * 
	 * @param pIter    the PathIterator to be parsed.
	 * @param params   an array with one elemet that will hold the calculated length
	 *                 of path;
	 ...
	 *                 elements of this path (see @link #shapeToRois()} for details;
	 * @param handles  a Vector of Point2D.Double objects representing vertices
	 *                 (segment joinings) and control points of the curves segments
	...
	boolean parsePath(PathIterator pIter, double[] params, Vector segments, Vector rois, Vector handles) {
		...
		int count = 0;// the number of segments in each subpath w/o SEG_CLOSE; resets to one after
				// each SEG_MOVETO
		int roiType = Roi.RECTANGLE;
		int segType;
	 ...
	 * <strong><code>this</code></strong> to the argument. <br>
	 ...
	 * passed in; a possible example is
	 * <code>setShape(ShapeRoi.cloneShape(shape))</code>.
	 * 
	...
	/**
	 * If this ROI consists of a single polygon, retuns the number of vertices,
	 * otherwise returns 4.
	 */
	public int size() {
