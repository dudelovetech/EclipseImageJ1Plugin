...
public class PlotWindow extends ImageWindow implements ActionListener, ItemListener, ClipboardOwner, ImageListener, RoiListener, Runnable {
	...
	/**
	 * Called by the constructor to generate the image the plot will be drawn
	 * on. This is a static method because constructors cannot call instance
	 * methods.
	 */
	...
	/**
	 * Sets the x-axis and y-axis range. Must be called before the plot is
	 * displayed.
	 */
	 ...
	 * Must be called before the plot is displayed. Note that there are more
	 * options available by using the methods of the Plot class instead.
	 * 
	 * @param x
	 *            the x-coodinates
	 * @param y
	 ...
	 * before the plot is displayed. Note that there are more options available
	 * by using the methods of the Plot class instead.
	 */
	 ...
	 * displayed. Note that there are more options available by using the
	...
	/**
	 * Draws a label. Must be called before the plot is displayed. Note that
	 ...
	 * black. Must be called before the plot is displayed. Note that there are
	 * more options available by using the methods of the Plot class instead.
	 */
	 ...
	 * that there are more options available by using the methods of the Plot
	 * class instead.
	 */
	...
	private static int COPY = 0, COPY_ALL = 1, SET_RANGE = 2, PREV_RANGE = 3, RESET_RANGE = 4, FIT_RANGE = 5, ZOOM_SELECTION = 6, AXIS_OPTIONS = 7, LEGEND = 8, RESET_PLOT = 9, FREEZE = 10, HI_RESOLUTION = 11, PROFILE_PLOT_OPTIONS = 12;
	// the following commands are disabled when the plot is frozen
	private static int[] DISABLED_WHEN_FROZEN = new int[] { SET_RANGE, PREV_RANGE, RESET_RANGE, FIT_RANGE, ZOOM_SELECTION, AXIS_OPTIONS, LEGEND, RESET_PLOT };
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (b == live)
			...
			boolean frozen = plot.isFrozen(); // prepare menu according to
												// 'frozen' state of plot
	...
	void showRangeArrows() {
		...
		hideRangeArrows(); // in case we have old arrows from a different plot
							// size or so
		rangeArrowRois = new Roi[4 * 2 + 1]; // 4 arrows per axis plus 1 'Reset'
												// icon
		int i = 0;
		int height = imp.getHeight();
		int arrowH = plot.topMargin < 14 ? 6 : 8; // height of arrows and
													// distance between them;
													// base is twice that value
		float[] yP = new float[] { height - arrowH / 2, height - 3 * arrowH / 2, height - 5 * arrowH / 2 - 0.1f };

		for (float x : new float[] { plot.leftMargin, plot.leftMargin + plot.frameWidth }) { // create
																								// arrows
																								// for
																								// x
																								// axis
		...
		for (float y : new float[] { plot.topMargin + plot.frameHeight, plot.topMargin }) { // create
																							// arrows
																							// for
																							// y
																							// axis
		...
		while (true) {
			...
			synchronized (this) {
				if (doUpdate) {
					...
					catch (InterruptedException e) { // interrupted tells the
														// thread to exit
						return;
					}
				}
