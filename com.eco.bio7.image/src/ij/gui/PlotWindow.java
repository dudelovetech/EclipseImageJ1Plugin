...
public class PlotWindow extends ImageWindow implements ActionListener, ItemListener, ClipboardOwner, ImageListener, RoiListener, Runnable {
	...
	public void draw() {
		...
		bottomPanel.setLayout(new GridLayout(10, 4, 0, 0));
		...
		/*
		 * JFrame fr = new JFrame(); fr.setContentPane(bottomPanel);
		 * fr.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); fr.setBounds(100, 100,
		 * 450, 172); fr.setAlwaysOnTop(true); fr.setVisible(true);
		 */
		...
		display.syncExec(new Runnable() {
			...
			public void run() {
				...
				custom2.setPanel(bottomPanel, id2, "Options " + plu.getTitle());
			}
		});
	}
	...
	private static int COPY = 0, COPY_ALL = 1, SET_RANGE = 2, PREV_RANGE = 3, RESET_RANGE = 4, FIT_RANGE = 5, ZOOM_SELECTION = 6, AXIS_OPTIONS = 7, LEGEND = 8, STYLE = 9, RESET_PLOT = 10, FREEZE = 11, HI_RESOLUTION = 12, PROFILE_PLOT_OPTIONS = 13;
	// the following commands are disabled when the plot is frozen
	private static int[] DISABLED_WHEN_FROZEN = new int[] { SET_RANGE, PREV_RANGE, RESET_RANGE, FIT_RANGE, ZOOM_SELECTION, AXIS_OPTIONS, LEGEND, STYLE, RESET_PLOT };
	...
	/**
	 * Creates an overlay with triangular buttons for changing the axis range limits
	 * and shows it
	 */
	 ...
	 * Index numbers start with 0 at the 'down' arrow of the lower side of the x
	 * axis and end with the up arrow at the upper side of the y axis.
	 */
