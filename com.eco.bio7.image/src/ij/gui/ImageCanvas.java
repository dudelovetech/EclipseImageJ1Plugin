...
public class ImageCanvas extends JPanel implements MouseListener, MouseWheelListener, MouseMotionListener, Cloneable {
	...
	/**
	 * Update this ImageCanvas to have the same zoom and scale settings as the
	 * one specified.
	 */
	...
	public void setSourceRect(Rectangle r) {
		...
		if (IJ.debugMode)
			IJ.log("setSourceRect: " + magnification + " " + (int) (srcRect.height * magnification + 0.5) + " " + dstHeight + " " + srcRect);
	}
	...
	void paintDoubleBuffered(Graphics g) {
		...
		try {
			...
			Image img = imp.getImage();
			if (img != null)
				offScreenGraphics.drawImage(img, 0, 0, srcRectWidthMag, srcRectHeightMag, srcRect.x, srcRect.y, srcRect.x + srcRect.width, srcRect.y + srcRect.height, null);
			if (overlay != null)
	...
	/**
	 * Converts a screen x-coordinate to a floating-point offscreen
	 * x-coordinate.
	 */
	...
	/**
	 * Converts a screen y-coordinate to a floating-point offscreen
	 * y-coordinate.
	 */
	...
	/**
	 * Converts a floating-point offscreen x-coordinate to a screen
	 * x-coordinate.
	 */
	 ...
	 * makes the source rectangle (srcRect) smaller and centers it on the
	 * position in the image where the cursor was when zooming has started. Note
	 * that sx and sy are screen coordinates.
	 */
	...
	/**
	 * Returns the size to which the window can be enlarged, or null if it can't
	 * be enlarged. <code>newWidth, newHeight</code> is the size needed for
	 * showing the full image at the magnification needed
	 */
	...
	/**
	 * Zooms out by making the source rectangle (srcRect) larger and centering
	 * it on (x,y). If we can't make it larger, then make the window smaller.
	...
	/**
	 * Sets the foreground drawing color (or background color if 'setBackground'
	 * is true) to the color of the pixel at (ox,oy).
	 */
	...
	protected void handleRoiMouseDown(MouseEvent e) {
		...
		if (roi != null) {
			...
			if (type == Roi.RECTANGLE && r.width == imp.getWidth() && r.height == imp.getHeight() && roi.getPasteMode() == Roi.NOT_PASTING && !(roi instanceof ImageRoi)) {
				imp.deleteRoi();
				return;
			}
	...
	/**
	 * Called by IJ.showStatus() to prevent status bar text from being
	 * overwritten until the cursor moves at least 12 pixels.
	 */
	...
	private boolean activateOverlayRoi(int ox, int oy) {
		...
		for (int i = o.size() - 1; i >= 0; i--) {
			...
			if (roi.contains(ox, oy) || (labels && labelRects != null && labelRects[i] != null && labelRects[i].contains(sx, sy))) {
				...
				if (!IJ.altKeyDown() && roi.getType() == Roi.COMPOSITE && roi.getBounds().width == imp.getWidth() && roi.getBounds().height == imp.getHeight())
					return false;
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (type > 0 && (type == Roi.POLYGON || type == Roi.POLYLINE || type == Roi.ANGLE || type == Roi.LINE) && roi.getState() == roi.CONSTRUCTING)
			roi.mouseMoved(e);
		else {
