...
public class Toolbar extends JPanel implements MouseListener, MouseMotionListener, ItemListener, ActionListener {
	...
	private static final String[] builtInTools = { "Arrow", "Brush", "Command Finder", "Developer Menu", "Flood Filler", "LUT Menu", "Overlay Brush", "Pencil", "Pixel Inspector", "Selection Rotator", "Spray Can", "Stacks Menu" };
	...
	private boolean installBuiltinTool(String label) {
		...
		if (label.startsWith("Arrow")) {
			...
			(new MacroInstaller()).installFile(getBio7Path() + "/macros/SprayCanTool.txt");
		} else if (label.startsWith("Developer Menu")) {
			(new MacroInstaller()).installFile(getBio7Path() + "/macros/DeveloperMenuTool.txt");
		} else if (label.startsWith("Stacks Menu")) {
			(new MacroInstaller()).installFile(getBio7Path() + "/macros/StacksMenuTool.txt");
		} else if (label.startsWith("LUT Menu")) {
			(new MacroInstaller()).installFile(getBio7Path() + "/macros/LUTMenuTool.txt");
