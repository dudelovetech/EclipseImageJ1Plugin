...
public class ProgressBar extends JPanel {
	 ...
	 * abs(<code>progress</code>) == 1 the bar is erased. The bar is updated only if
	 * more than 90 ms have passed since the last call. Does nothing if the ImageJ
	 * window is not present.
	 * 
	 * @param progress Length of the progress bar to display (0...1). Using
	 *                 <code>progress</code> with negative sign (0 .. -1) will
	 *                 regard subsequent calls with positive argument as
	 *                 sub-ordinate processes that are displayed as moving dot.
	 */
	 ...
	 * length. Use a negative <code>currentIndex</code> to show subsequent plugin
	 * calls as moving dot. The bar is erased if
