...
public class IJMacroFunctions {
	...
	public static String functions=""+
			...
			"Dialog.addSlider(\"Label\", min, max, default)####Adds a slider controlled numeric field to the dialog, using the specified label, and min, max and default values (example). Values with decimal points are used when (max-min)<=5 and min, max or default are non-integer. Requires 1.45f."+linSeparator + 
			...
			"Dialog.addCheckboxGroup(rows, columns, labels, defaults)####Adds a rowsxcolumns grid of checkboxes to the dialog, using the specified labels and default states (example)."+linSeparator + 
			...
			"File.openDialog(title)####Displays a file open dialog and returns the path to the file choosen by the user (example). The macro exits if the user cancels the dialog."+linSeparator + 
			...
			"Fit.doFit(equation, xpoints, ypoints)####Fits the specified equation to the points defined by xpoints, ypoints. Equation can be either the equation name or an index. The equation names are shown in the drop down menu in the Analyze>Tools>Curve Fitting window. With ImageJ 1.42f or later, equation can be a string containing a user-defined equation (example)."+linSeparator + 
			"Fit.doFit(equation, xpoints, ypoints, initialGuesses)####Fits the specified equation to the points defined by xpoints, ypoints, using initial parameter values contained in initialGuesses, an array equal in length to the number of parameters in equation (example)."+linSeparator + 
			...
			"Fit.f(x)####Returns the y value at x (example)."+linSeparator + 
			...
			"getFileList(directory)####Returns an array containing the names of the files in the specified directory path. The names of subdirectories have a \"/\" appended. For an example, see the ListFilesRecursively macro."+linSeparator + 
			"getFontList()####Returns an array containing the names of available system fonts (example)."+linSeparator+
			...
			"getInfo(\"command.name\")####Returns the name of the most recently invoked command. The names of commands invoked using keyboard shortcuts are preceded by \"^\" (example)."+linSeparator + 
			...
			"getList(\"threshold.methods\")####Returns a list of the available automatic thresholding methods (example)."+linSeparator + 
			"getList(\"LUTs\")####Returns, as an array, a list of the LUTs in the Image>Lookup Tables menu (example)."+linSeparator+
			...
			"IJ.pad(n, length)####Pads 'n' with leading zeros and returns the result (example). Requires 1.45d."+linSeparator + 
			"IJ.redirectErrorMessages()####Causes next image opening error to be redirected to the Log window and prevents the macro from being aborted (example). Requires 1.43n."+linSeparator + 
			...
			"makePoint(x, y, options)####Creates a point selection at the specified location, with the type ('hybrd', 'cross', 'dot' or 'circle'), color ('red', 'white', etc.) and size ('tiny', 'small', 'medium', 'large' or 'extra large') of the point defined in the 'options' string (example). The point is added to an overlay if the options string contains 'add'. Requires 1.52i."+linSeparator + 
			...
			"Overlay.setPosition(n)####Sets the stack position (slice number) of the last item added to the overlay (example)."+linSeparator + 
			...
			"Overlay.drawString(\"text\", x, y)####Draws text at the specified location and adds it to the overlay. Use setFont() to specify the font and setColor to set specify the color (example)."+linSeparator + 
			"Overlay.drawString(\"text\", x, y, angle)####Draws text at the specified location and angle and adds it to the overlay (example)."+linSeparator + 
			...
			"Plot.setBackgroundColor(color)####Sets the background color of the plot frame (example). Requires 1.49h."+linSeparator + 
			...
			"Plot.getValues(xpoints, ypoints)####Returns the values displayed by clicking on \"List\" in a plot or histogram window (example)."+linSeparator + 
			...
			"Roi.getSplineAnchors(x, y)####Returns the x and y coordinates of the anchor points of a spline fitted selection (example)."+linSeparator + 
			"Roi.setPolygonSplineAnchors(x, y)####Creates or updates a spline fitted polygon selection (example)."+linSeparator + 
			"Roi.setPolylineSplineAnchors(x, y)####Creates or updates a spline fitted polyline selection (example)."+linSeparator + 
			...
			"Stack.setOrthoViews(x, y, z)####If an Orthogonal Views is active, its crosspoint is set to x, y, z (example)."+linSeparator + 
